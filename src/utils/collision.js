...
export function buildHeightField(positionGroups, transform, { cellSize }) {
	...
	for (const positions of positionGroups) {
		for (let i = 0; i < positions.length; i += 9) {
			const ax = positions[i] * s + tx,
				az = positions[i + 2] * s + tz;
			const bx = positions[i + 3] * s + tx,
				bz = positions[i + 5] * s + tz;
			const cx = positions[i + 6] * s + tx,
				cz = positions[i + 8] * s + tz;

			const topY =
				Math.max(positions[i + 1], positions[i + 4], positions[i + 7]) *
					s +
				ty;

			const triMinX = Math.min(ax, bx, cx),
				triMaxX = Math.max(ax, bx, cx);
			const triMinZ = Math.min(az, bz, cz),
				triMaxZ = Math.max(az, bz, cz);

			const colLo = Math.max(0, Math.floor((triMinX - minX) / cellSize));
			const colHi = Math.min(
				cols - 1,
				Math.floor((triMaxX - minX) / cellSize),
			);
			const rowLo = Math.max(0, Math.floor((triMinZ - minZ) / cellSize));
			const rowHi = Math.min(
				rows - 1,
				Math.floor((triMaxZ - minZ) / cellSize),
			);
...
export function maxSurfaceHeightAt(heightField, x, z, heading, samples) {
	const fx = Math.cos(heading),
		fz = -Math.sin(heading); // frente (avanço)
	const lx = -fz,
		lz = fx; // lateral (perpendicular)

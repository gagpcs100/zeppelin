...
export async function createScene(gl) {
  ...
  const lamps = lc.lampPositions.map(([x, z]) => ({
    position: [x, surfaceHeightAt(world.heightField, x, z) + lc.lampHeight, z],
    color: lampColor,
    range: lc.lampRange,
  }));
  const lampMarker = twgl.primitives.createSphereBufferInfo(gl, lc.lampMarkerSize, 12, 8);

  const shadow = createShadow(gl);

  return {
    textures, world, zeppelin, skybox, sky, dayCycle, autopilot,
    lamps, lampMarker, shadow,
